...
def dump_author_embs():
    ...
    with lc_feature.db.begin() as txn:
        for k in txn.cursor():
            ...
            if pid_order in emb_model.entity_embeddings:
                # include entity embedding
                ent_embedding = emb_model.project_entity_embeddings(pid_order)
                # embedding summary
                cur_emb = np.mean([cur_emb, ent_embedding], axis=0)

...
class EmbeddingModel:
    ...
    def __init__(self, name="aminer"):
        self.model = None
        self.name = name
        self.entity_embeddings = pickle.load(open('aminer_entities_doc_embeddings.p', 'rb'))
    ...
    def train(self, wf_name, size=EMB_DIM):
        ...
        self.model = Word2Vec(
            data, size=size, window=5, min_count=MIN, workers=20,
        )
    ...
    def project_entity_embeddings(self, pid):
        """
        combined entity embeddings
        :param pid: identifier of the project (key)
        """
        return self.entity_embeddings[pid]
